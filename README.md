...
| `update_recipe` | Targeted edits: name, description, servings, times, add/remove keywords. Keyword edits require `expected_revision`; successful writes return a fresh `edit_revision` for the next guarded edit. |
| `set_recipe_steps` | Replace a recipe's steps/ingredients with a non-empty list — read `get_recipe`'s `steps[]` and `edit_revision`, edit, pass back with `expected_revision`. Successful writes return a fresh `edit_revision`; this tool does not clear all steps. |
...
| `log_cooked` / `get_cook_log` | Record or review cook history and ratings. |
...
| Tool | Purpose |
|---|---|
| `plan_meal` / `get_meal_plan` / `update_meal_plan_entry` / `remove_meal_plan_entry` | Manage the meal-plan calendar (meal type by name, recipe by name/id). `get_meal_plan` returns entries under `entries`. |
...
| `get_inventory` | Read real inventory entries with amount/unit/location/expiry metadata. This is read-only; inventory writes remain intentionally designed for a later sprint. |
| `list_taxonomy` / `create_taxonomy` / `rename_taxonomy` | List, create and rename/update keywords, foods or units (`kind`) with ids; plural aliases `keywords`, `foods`, `units` are accepted. |
...
spaces, uploaded user files, per-user preferences and logs are intentionally
hidden from the generic tools because their responses are returned to the model.
