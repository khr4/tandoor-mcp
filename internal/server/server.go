...
func (h *handlers) register(s *mcp.Server) {
	...
	addTool(s, h, &mcp.Tool{Name: "set_recipe_image", Description: "Set a recipe's image from exactly one source: image_url for a public http/https URL, or image_path for a regular local file within the configured image directory. URL credentials, localhost, private, link-local, and internal hosts are rejected. If result status is outcome_unknown, re-read the recipe before retrying; the write may have committed."}, h.setRecipeImage)
